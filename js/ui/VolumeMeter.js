...
export default class VolumeMeter {
    ...
    resize() {
        ...
        this.barWidth =
            (this.canvas.width - this.barGap * (this.bars - 1)) /
            this.bars;
    }
    ...
    animate() {
        this.level += (this.target - this.level) * 0.15;
    }
    ...
    drawBars() {
        ...
        for (let i = 0; i < this.bars; i++) {
            const variation =
                0.85 +
                0.15 * Math.sin(performance.now() * 0.01 + i);

            const h = this.level * variation * this.maxHeight;
            const x = i * (this.barWidth + this.barGap);
            const y = this.canvas.height - h;

            ctx.fillRect(x, y, this.barWidth, h);
        }
    }

    drawPeak() {
        const ctx = this.ctx;
        const y = this.canvas.height - this.level * this.maxHeight;
